...
create table book_chapter (
    ...
    del_flag tinyint default 0 not null,
    constraint book_chapter_pk unique (book_id, chapter_num)
) comment '小说章节';

create index idx_bookId on book_chapter (book_id);
...
create table book_content (
    ...
    del_flag tinyint default 0 not null,
    constraint uk_chapterId unique (chapter_id)
) comment '小说内容';
...
create index idx_createTime on book_info (create_time);

create index idx_lastChapterUpdateTime on book_info (last_chapter_update_time);

create table carousel_banners (
    id bigint unsigned auto_increment primary key,
    book_id int unsigned not null,
    title varchar(50) not null,
    bio varchar(100) not null comment '简介',
    image_url varchar(255) not null,
    sort_order tinyint not null comment '排序权重，越大越靠前',
...
create table chapter_summary (
    ...
    constraint chapter_summary_pk unique (
        book_id,
        chapter_id,
        segment_num
    )
...
create index segment_comment_segment_num_chapter_id_index on segment_comment (segment_num, chapter_id);
...
create table user_bookshelf (
    ...
    constraint pk_id unique (id),
    constraint uk_userId_bookId unique (user_id, book_id)
) comment '用户书架';
...
create table user_comment (
    ...
    update_time datetime null comment '更新时间',
    constraint pk_id unique (id),
    constraint uk_bookId_userId unique (book_id, user_id)
) comment '用户评论';
...
create table user_info (
    ...
    role varchar(16) default 'user' not null comment 'user普通用户; admin管理员',
    create_time datetime default(now()) null comment '创建时间',
    update_time datetime default(now()) null comment '更新时间',
    constraint pk_id unique (id),
