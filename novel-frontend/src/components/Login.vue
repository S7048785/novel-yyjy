...
const captchaUrl = ref(import.meta.env.VITE_APP_API_URL + '/captcha' + '?' + Date.now());
const setCaptchaRandom = () => {
	captchaUrl.value = import.meta.env.VITE_APP_API_URL + '/captcha' + '?' + Date.now();
};
...
const loginSubmit = async () => {
	// 参数校验
	if (!loginForm.value.email || !loginForm.value.password || !loginForm.value.captcha) {
...
const registerSubmit = async () => {
	// 数据校验
	if (!registerForm.value.email || !registerForm.value.password || !registerForm.value.nickName || !registerForm.value.captcha) {
		message.error('请填写完整信息');
		return;
	}
...
<template>
	<a-modal
			closable
			style="top: 4em"
			:body-style="{'height': toggle ? '530px' : '410px', 'transition-duration': '0.3s', 'overflow': 'hidden', 'position': 'relative'}"
			:footer="false"
			:maskStyle="{'backdrop-filter': 'blur(3px)', 'background-color':`${isDark ? 'rgba(0, 0, 0, 0.3)' : 'rgba(255, 255, 255, 0.3)'}`}"
			@cancel="close"
		...
		<template #closeIcon>
					...
					<div v-if="!toggle" class="absolute w-90% m-6 bg-white">
						...
						<a-form @submit="loginSubmit" :model="loginForm" class="flex flex-col">
							...
							<a-form-item name="email">
								<a-input v-model:value="loginForm.email" type="email"
							...
							<a-form-item name="password">
								<a-input v-model:value="loginForm.password" autocomplete="off" type="password" class="bg-gray-100 text-gray-900 rounded-md p-2 mt-4 border-0! focus:bg-gray-200 focus:outline-none focus:ring-1 focus:ring-blue-500 transition ease-in-out duration-150" placeholder="密码" />
							</a-form-item>
							<!--				TODO: 验证玛-->
							<div class="inline-flex items-center mb-4 justify-end">
								<img class="w-25 h-10 rounded-md mr-2 border-black border-1" @click="setCaptchaRandom" :src="captchaUrl" alt="">
								<a-form-item name="captcha" class="mb-0">
									<a-input v-model:value="loginForm.captcha" type="text"
							...
							<div class="inline-flex items-center justify-between my-4">
								...
								<p class="text-gray-900">
									还没有账号?
									<a @click="toggle = true" href="#" class="text-blue-500 mx-2 hover:underline">注册</a>
								</p>
							</div>
				...
				<transition name="sign-up">
					<div v-if="toggle" class="m-6 absolute w-90% ">
						...
						<a-form  @submit="registerSubmit" :model="registerForm" class="flex flex-col">
							<a-input v-model:value="registerForm.nickName" type="text"
								 class="bg-gray-100 text-gray-900 border-0! rounded-md p-2 mb-8 focus:bg-gray-200 focus:outline-none focus:ring-1 focus:ring-blue-500 transition ease-in-out duration-150"
									 placeholder="昵称"/>
							<a-input v-model:value="registerForm.email" type="email"
									 class="bg-gray-100 text-gray-900 border-0! rounded-md p-2 mb-8 focus:bg-gray-200 focus:outline-none focus:ring-1 focus:ring-blue-500 transition ease-in-out duration-150"
									 placeholder="邮箱"/>
							<a-input v-model:value="registerForm.password" type="password" autocomplete="off"
									 class="bg-gray-100 text-gray-900 border-0! rounded-md p-2 mb-8 focus:bg-gray-200 focus:outline-none focus:ring-1 focus:ring-blue-500 transition ease-in-out duration-150"
										 placeholder="密码" />
							<a-input v-model:value="registerForm.confirmPassword" type="password" autocomplete="off"
							...
							<!--				TODO: 验证玛-->
							<div class="inline-flex items-center mb-4 justify-end">
								<a-input v-model:value="registerForm.captcha" type="text"
								...
								<p class="text-gray-900 text-center my-4">
									已有账号?
									<a @click="toggle = false" href="#" class="text-blue-500 mx-2 hover:underline">登录</a>
								</p>
							<button type="submit" class="bg-gradient-to-r! from-indigo-500 to-blue-500 text-white font-bold py-2 px-4 rounded-md mt-4 hover:bg-indigo-600 hover:to-blue-600 transition ease-in-out duration-150">
								注册
							</button>
