...
import type {BookChapterView, BookContentView} from "@/type/book.ts";
...
const params: { bookId: string, chapterId: string } = route.params;
// 夜间模式
...
const sidebarList = reactive([
	...
	},
	{
		...
		onClick: () => {
			...
			catalogueModal.value.querySelector('.ant-modal-content').style.background = isDark.value ? '#262626' : '#ffffff';
		}
	}
])
...
const getBookContent = async () => {
	bookContentView.value = (await getBookContentAPI(params.bookId, params.chapterId)).data;
...
<template>
	...
	<div class="bg-neutral-200 dark:bg-[#202020FF]">

		<div class="w280 mx-auto bg-neutral-200 dark:bg-[#202020FF]">
			<!-- 侧边栏 -->
			<div class="fixed flex flex-col z-20 ml-[-20px] h-screen top-40">
					<span v-for="item in sidebarList"
								@click="item.onClick"
						...
						<Icon>
							<template #component>
								<svg width="2rem" height="2em" viewBox="0 0 1024 1024"
										 xmlns="http://www.w3.org/2000/svg">
									<path
											:d="item.d"
											:fill="`${isDark ? '#A3A3A3' : '#263140'}`"></path>
								</svg>
							</template>
						</Icon>
						{{ item.name }}
			...
			<div v-if="loading" class="h-228 w240 bg-white dark:bg-neutral-800 flex items-center justify-center p-5 mx-auto">
				<a-spin tip="加载中..." size="large"/>
			</div>
			<div v-else class="w240 mx-auto h-full pt4 bg-neutral-100 dark:bg-neutral-800">
				...
				<div class="flex justify-between border-b dark:border-gray-7">
					...
					<div class="pr6">
						<a-dropdown v-if="userStore.user">
							<div class="p-1 rounded-lg hover:bg-neutral-100 dark:hover:bg-neutral-800">
								<a-avatar size="large" :src="userStore.user?.userPhoto"></a-avatar>
								<span class="ml-4 dark:text-gray">{{ userStore.user?.nickName }}</span>
							</div>
							...
							<template #overlay>
								<a-menu class="dark:bg-neutral-700">
									...
									<a-menu-item class="">
										<div class="text-center">
											<a @click="logout" href="javascript:;" class="dark:text-gray-400! hover:text-red-5!">退出登录</a>
										</div>
									</a-menu-item>
								</a-menu>
							</template>
						</a-dropdown>
						<a-button v-else @click="emitter.emit('open-login')" class="text-gray p-0 hover:text-blue!" type="text">登录</a-button>
					</div>
				</div>
				<!--			面包屑-->
				<div class="py6 border-b dark:border-gray-7 pl6">
					<a-breadcrumb>
						<template #separator><span class="dark:text-neutral-500"> -> </span></template>
						<a-breadcrumb-item>
							<router-link to="/" class="dark:text-neutral-500 dark:hover:text-neutral-400">
								<span class="text-neutral-500 dark:text-neutral-500">首页</span>
							</router-link>
						</a-breadcrumb-item>
						<a-breadcrumb-item>
							<router-link :to="{
								...
								<span class="text-neutral-500 dark:text-neutral-500">{{bookContentView?.categoryName}}</span>
							</router-link>
						</a-breadcrumb-item>
						<a-breadcrumb-item>
				...
				<div class="pb6 mt6">
					<!--				头部-->
					<div class="text-center">
						<p class="text-2xl mb4 dark:text-neutral-400">{{ bookContentView?.chapterName }}</p>
						<p class="text-sm inline-flex gap-6 text-gray-500 dark:text-neutral-400">
							<span class="inline-flex items-center">
								<Icon class="mr-1">
									<template #component>
										<svg t="1752059332317" class="size-4" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="6958"><path d="M895.887 166.804c-15.842 0-28.655-12.813-28.655-28.667 0-15.854 12.813-28.667 28.655-28.667 15.854 0 28.667-12.814 28.667-28.667s-12.813-28.667-28.667-28.667H207.953c-47.471 0-85.991 38.485-85.991 86.001v773.925c0 31.662 25.639 57.334 57.323 57.334h687.946c31.685 0 57.322-25.672 57.322-57.334V195.471c0.001-15.854-12.813-28.667-28.666-28.667z m0.619 57.334v706.435H153.309V199.614c4.2 5.765 28.825 24.523 54.644 24.523h109.174v-28.825H207.953c-15.786 0-54.644-11.665-54.644-57.177 0-47.088 38.857-58.123 55.33-58.123h691.92c-12.025 7.026-53.776 11.395-53.776 58.123 0 34.703 42.179 50.601 49.723 57.177H499.68v28.825h396.826z" :fill="`${isDark ? '#A3A3A3' : '#263140'}`" p-id="6959"></path><path d="M293.943 52.135v458.63l114.658-114.646 114.657 114.646V52.135H293.943z m203.339 395.621l-88.681-87.916-87.983 87.916V78.798h176.665v368.958z" :fill="`${isDark ? '#A3A3A3' : '#263140'}`" p-id="6960"></path></svg>
									</template>
								</Icon>
								{{ bookContentView?.bookName }}
							</span>
							<span class="inline-flex items-center">
								<Icon class="mr-1">
									<template #component>
										<svg t="1752058993733" class="size-4" viewBox="0 0 1024 1024" version="1.1"
												 xmlns="http://www.w3.org/2000/svg" p-id="5493">
											<path
							...
							<span class="inline-flex items-center">
								<Icon class="mr-1">
									<template #component>
										<svg t="1752059552103" class="size-4" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="14893"><path :fill="`${isDark ? '#A3A3A3' : '#263140'}`" d="M512 101.376a410.624 410.624 0 1 1 0 821.248 410.624 410.624 0 0 1 0-821.248z m0 74.666667a336.042667 336.042667 0 1 0 0 672 336.042667 336.042667 0 0 0 0-672.085334z m149.333333 149.333333a37.290667 37.290667 0 0 1 6.741334 73.984l-6.741334 0.682667H549.290667v298.666666a37.290667 37.290667 0 0 1-73.984 6.656l-0.682667-6.741333v-298.666667H362.666667a37.290667 37.290667 0 0 1-6.741334-73.984l6.741334-0.597333h298.666666z" p-id="14894"></path></svg>
									</template>
								</Icon>
							...
							<span class="inline-flex items-center">
								<Icon class="mr-1">
									<template #component>
									<svg t="1752059800853" class="size-4" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="3525"><path d="M512 0C229.225814 0 0 229.248624 0 512s229.225814 512 512 512 512-229.225814 512-512S794.774186 0 512 0z m0 937.524225v0.980865c-235.555813 0-426.493685-190.937872-426.493685-426.50509s190.926466-426.562117 426.493685-426.562117 426.493685 190.994899 426.493685 426.562117-215.037491 425.51282-426.493685 425.51282z m213.281059-425.524225h-191.952953V255.994297c0-17.906485-18.453944-31.935132-36.497294-31.935131-26.049943 0-48.826535 14.051458-48.826535 31.935131v298.67332c0 17.906485 24.624268 42.667617 42.667617 42.667617h234.609165c18.043349 0 26.722862-20.005079 26.722862-37.922969 0.057027-17.929295-8.622486-47.412265-26.722862-47.412265z" :fill="`${isDark ? '#A3A3A3' : '#263140'}`"p-id="3526"></path></svg>
									</template>
								</Icon>
								{{ dayjs(bookContentView?.createTime).format('YYYY年MM月DD日 HH:mm') }}
							</span>
						</p>
					</div>
					<!--				内容-->
					<div class="mt-10 w-10/12 mx-auto text-lg dark:text-neutral-400">
						<div v-html="bookContentView?.content"></div>
					</div>
					<!--				底部-->
					<div class="px6 pt-10 mt4 flex gap-10 items-center justify-center">
						<router-link v-if="bookContentView?.prevChapterId"
												 :to="`/read/${params.bookId}/${bookContentView?.prevChapterId}`" type="text"
												 class="rounded-full bg-white px30 py4 hover:bg-neutral-200 transition-duration-300 content-center">
							上一章
						</router-link>
						<router-link v-if="bookContentView?.nextChapterId"
												 :to="`/read/${params.bookId}/${bookContentView?.nextChapterId}`" type="text"
		...
		<div ref="catalogueModal">
			<!-- 目录 -->
			<a-modal
					v-model:visible="catalogueVisible"
					:get-container="() => $refs.catalogueModal"
					closable
					:forceRender="true"
					style="top: 20px;"
					:width="880"
				...
				<template #title>
					<div class="dark:bg-neutral-800 flex justify-between">
						<div>
							...
							<span class="text-gray dark:text-neutral-500"> 连载中 · 共 {{ bookChapter.total }} 章</span>
						</div>
						<div class="inline-flex mr-10">
							<a-button @click="changeOrder" type="text" class="inline-flex items-center px-1 dark:hover:bg-neutral-700">
								<Icon>
									<template #component>
										<svg t="1752056997879" class="size-4 p0" viewBox="0 0 1024 1024" version="1.1"
												 xmlns="http://www.w3.org/2000/svg" p-id="3699">
											<path :fill="`${isDark ? '#A3A3A3' : '#263140'}`"
								...
								<span class="dark:text-neutral">{{ chapterOrderDesc ? '正序' : '倒序' }}</span>
							</a-button>
						</div>
					</div>
				</template>
				...
				<div id="catalogue" class="mt-4 flex px-6 flex-wrap h-150 overflow-y-scroll">
					<div v-for="item in bookChapter.bookChapters"
						...
						<router-link :to="`/read/${params.bookId}/${item.id}`"
...
#catalogue {
	--scrollbar-bg: var(--bg-color); /* 显式传递变量 */
